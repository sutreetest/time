...
#------------------------------- CHECK SIDER --------------------------------
            if msg.text.lower() in ["นับ"]:
                if msg.toType == 2:
                    cl.sendText(msg.to, "เริ่มเช็คค่า")
                    try:
...
#-------------------------------------------------------------------------#									
	...
	if op.type == 15:
        ...
        if op.type == 17:
            random.choice(KAC).sendText(op.param1, cl.getContact(op.param2).displayName + ", สวัสดี ยินดีที่ได้รู้จัก ")
            print op.param3 + "has left the group"
